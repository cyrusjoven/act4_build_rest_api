...
productRouter.get('/products', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(error); // Simplified error response
  }
});
...
productRouter.get("/product/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (!product) {
      return res.status(StatusCodes.NOT_FOUND).json({ error: "Product does not exist" }); // Fixed error key
    }

    return res.status(StatusCodes.OK).json({ product }); // Corrected response
  } catch (error) {
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(error); // Simplified error response
  }
});
